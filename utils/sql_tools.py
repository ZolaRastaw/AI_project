...
def insert_row(email, Title, password, user_name=config.mysql_user, mysql_password=config.mysql_password, host_name=config.mysql_host, db_name=config.db_name, table_name=config.table_name):
    # Define our connection string
    conn = MySQLdb.connect(user=user_name, password=mysql_password, host=host_name, database=db_name)
    try:
        cursor = conn.cursor()
        cursor.execute("""insert into %s (email, Title, password) values ('%s', '%s', '%s'); """ %(table_name, email, Title, password))
        conn.commit()
    finally:
        conn.close()
    print('Row inserted')
